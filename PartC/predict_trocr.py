"""End-to-end pipeline using fine-tuned TrOCR-CharVocab as the recognizer.

Image -> Part A (YOLO) -> crop -> TrOCR -> text.

Usage:
    python -m PartC.predict_trocr \\
        --partA PartA/runs/yolov8s_1280/weights/best.pt \\
        --partC PartC/runs/trocr_charvocab/best \\
        --images Dataset/CheckImages \\
        --out predictions_legal_trocr.txt
"""
...
from PartC.evaluate_trocr import load_model_and_processor
...
@torch.no_grad()
def recognize(crop: Image.Image, model, processor, device, gen_kwargs: dict) -> str:
...
def main() -> None:
    ...
    ap.add_argument("--partC", type=Path, required=True, help="TrOCR fine-tune dir (best/)")
    ap.add_argument("--images", type=Path, required=True)
    ap.add_argument("--out", type=Path, required=True)
    ap.add_argument("--save-crops", type=Path, default=None,
                    help="If set, also save predicted-bbox crops here.")
    ...
    print(f"Loaded TrOCR from {args.partC}")
    ...
    with args.out.open("w", encoding="utf-8") as f:
        for img_path in images:
            ...
            text = recognize(crop, model, processor, device, gen_kwargs)
